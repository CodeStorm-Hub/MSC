...
Change log (from original quotation-html-generator agent)
- Generalized agent name and purpose (quotation -> docs).
- Added frontmatter-driven configuration and repository-aware behavior.
- Improved handling of local images (placeholder with comment) and signature placeholders.
- Better print rules, A4 @page defaults and page-break heuristics.
- Explicit error handling and commit message conventions.

If you want, I can:
- 1) Create/update this file at files/.github/agents/quotation-html-generator.agent.md in the repo with the contents above and commit with message "chore(docs): make generic docs-html-generator agent", or
- 2) Open a PR that replaces the original .github/agents/quotation-html-generator.agent.md with this generic agent and updates references in the repository to use the new agent name, or
- 3) Produce a concrete example HTML output for docs/Quotation.md (read the file and generate files/docs/Quotation.html) using the new generic rules.

Which action would you like me to take next?